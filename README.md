...
- transformer une tâche existante de priorité élevée en `UrgentTask` depuis le menu ;
...
- `repositories/` : contrat générique `Repository<T>`, persistance JSON et implémentation mémoire interchangeable.
...
Une `UrgentTask` est une spécialisation de `Task` : sa priorité est toujours élevée, elle est enregistrée avec le type `URGENTE` et apparaît avec le marqueur `🚨` dans la liste. Cela la distingue d'une tâche standard simplement classée en priorité élevée.
